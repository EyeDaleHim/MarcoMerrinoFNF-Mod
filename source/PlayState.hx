...
class PlayState extends MusicBeatState
{
	...
	public static var versionSong:String = '-';
	...
	override public function create()
	{
		...
		healthBar.createFilledBar(CharacterColor.color(dad.curCharacter), CharacterColor.color(boyfriend.curCharacter));
		add(healthBar);
	...
	function startSong():Void
	{
		...
		if (!paused)
			FlxG.sound.playMusic(Paths.inst(PlayState.SONG.song + versionSong), 1, false);
	...
	function updateAccuracy():Float
	{
		...
		{
			accuracy = totalRanksHit / (totalNotesHit + misses) * 100;
		...
		else if (accuracyType == 'complex')
		{
			...
			if (curNoteType != 'held')
				accuracy = totalRanksHit / (totalNotesHit + misses + AccuracyHelper.complexCalc(lastNoteDiff, Conductor.safeZoneOffset / 166)) * 100;
	...
	private function generateSong(dataPath:String):Void
	{
		...
		if (SONG.needsVoices)
			vocals = new FlxSound().loadEmbedded(Paths.voices(PlayState.SONG.song + versionSong));
		else
			vocals = new FlxSound();
	...
	private function popUpScore(strumtime:Float, isSustainNote:Bool = false):Void
	{
		...
		for (i in seperatedScore)
		{
			...
			if (combo >= 10 || combo == 0 && totalNotesHit > 2)
				add(numScore);
	...
	private function keyShit():Void // I've invested in emma stocks
	{
		...
		notes.forEachAlive(function(daNote:Note)
		{
			if (downscroll && daNote.y > strumLine.y || !downscroll && daNote.y < strumLine.y)
			{
				...
				if (Settings.botplay && daNote.canBeHit && daNote.mustPress || Settings.botplay && daNote.tooLate && daNote.mustPress)
				{
					goodNoteHit(daNote);
	...
	override function beatHit()
	{
		...
		if (SONG.song.toLowerCase() == 'marcovid')
		{
			switch (curBeat)
			{
				case 64 | 260:
		...
		if (camZooming && FlxG.camera.zoom < 1.35 && curBeat % camBeat == 0 && !milfActive && curBeat > 64)
		{
			FlxG.camera.zoom += 0.015;
			camHUD.zoom += 0.03;
		}
